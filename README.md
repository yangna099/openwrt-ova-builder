...
./build-openwrt-wireguard-client.sh \
  ...
  --wg-quick-config clients/TGW_client.conf
```

The client build requires a `wg-quick` configuration previously generated by
`playbooks/create-wireguard-client.yml`. It applies that configuration before
packaging the OVA, so the imported appliance is ready to establish its
`wgclient` tunnel after its WAN interface obtains connectivity.
