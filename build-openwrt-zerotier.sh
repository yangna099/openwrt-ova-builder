#!/usr/bin/env bash
# Build an OpenWrt ZeroTier gateway and package it as an OVA.

set -euo pipefail

usage() {
    cat <<'EOF'
Usage: ./build-openwrt-zerotier.sh --ova FILE [options]
...
Build steps:
  ...
  5. Install ZeroTier and its runtime dependencies.
...
Options:
  ...
  --network FILE           OpenWrt network UCI file (default: networks/zerotier.conf)
...
disk_dir='disk'
keys_dir='keys'
network_file="${project_dir}/networks/zerotier.conf"
ova_file=''
ssh_port='22222'
luci_port='8080'
boot_timeout='180'
...
for required_file in \
    ...
    playbooks/install-zerotier.yml playbooks/configure-zerotier.yml \
...
echo '==> Installing ZeroTier and runtime dependencies'
"${project_dir}/run-ansible.sh" "${project_dir}/playbooks/install-zerotier.yml"
...
<ovf:Envelope xmlns:ovf="http://schemas.dmtf.org/ovf/envelope/1" xmlns:rasd="http://schemas.dmtf.org/wbem/wscim/1/cim-schema/2/CIM_ResourceAllocationSettingData" xmlns:vssd="http://schemas.dmtf.org/wbem/wscim/1/cim-schema/2/CIM_VirtualSystemSettingData" xmlns:vmw="http://www.vmware.com/schema/ovf">
  ...
  <ovf:NetworkSection>
    ...
    <ovf:Network ovf:name="MANAGEMENT"><ovf:Description>Ansible management adapter</ovf:Description></ovf:Network>
  </ovf:NetworkSection>
  <ovf:VirtualSystem ovf:id="openwrt-zt-pbr">
    <ovf:Info>OpenWrt ZeroTier gateway</ovf:Info>
    <ovf:Name>openwrt-zt-pbr</ovf:Name>
    ...
    <ovf:VirtualHardwareSection>
      ...
      <ovf:System><vssd:ElementName>Virtual Hardware Family</vssd:ElementName><vssd:InstanceID>0</vssd:InstanceID><vssd:VirtualSystemIdentifier>openwrt-zt-pbr</vssd:VirtualSystemIdentifier><vssd:VirtualSystemType>virtualbox-2.2</vssd:VirtualSystemType></ovf:System>
      ...
      <ovf:Item><rasd:InstanceID>7</rasd:InstanceID><rasd:ResourceType>10</rasd:ResourceType><rasd:ResourceSubType>E1000</rasd:ResourceSubType><rasd:Connection>MANAGEMENT</rasd:Connection></ovf:Item>
    </ovf:VirtualHardwareSection>
  </ovf:VirtualSystem>
</ovf:Envelope>
EOF
...
(
    cd "$work_dir"
    sha256sum "$ovf_name" "$vmdk_name" | while read -r sum file; do
        printf 'SHA256(%s)= %s\n' "$file" "$sum"
    done >"$manifest_name"
)
...
printf '%s\n' "$user_configuration_output"
echo
echo 'ZeroTier is installed but not joined to a network.'
echo 'The OVA includes eth2 for Ansible management. Connect it to the PVE internal bridge.'
echo 'After importing the OVA, run:'
echo '  playbooks/configure-zerotier.yml'
